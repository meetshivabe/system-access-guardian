-- ========================================
-- COMPLETE BOOKING SYSTEM MIGRATION (FIXED)
-- Run this SQL in your Supabase SQL Editor
-- ========================================

-- Step 1: Add booking columns to systems table
...
-- Step 2: Add booking columns to subsystems table  
ALTER TABLE public.subsystems
ADD COLUMN IF NOT EXISTS booking_start TIMESTAMP WITH TIME ZONE,
ADD COLUMN IF NOT EXISTS booking_end TIMESTAMP WITH TIME ZONE;

-- Step 3: Create bookings table for future bookings
CREATE TABLE IF NOT EXISTS public.system_bookings (
...
-- Step 4: Create subsystem bookings table
CREATE TABLE IF NOT EXISTS public.subsystem_bookings (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
-- Step 6: RLS policies for system_bookings
...
-- Step 7: RLS policies for subsystem_bookings
...
-- Step 8: Create function to check booking conflicts
...
-- Step 9: Create function to check subsystem booking conflicts
...
-- Step 10: Create trigger to prevent overlapping bookings
...
-- Step 11: Create function to automatically lock/unlock based on bookings
...
CREATE INDEX IF NOT EXISTS idx_system_bookings_system_id ON public.system_bookings(system_id);
CREATE INDEX IF NOT EXISTS idx_system_bookings_user_id ON public.system_bookings(user_id);
CREATE INDEX IF NOT EXISTS idx_system_bookings_status ON public.system_bookings(status);
CREATE INDEX IF NOT EXISTS idx_system_bookings_times ON public.system_bookings(booking_start, booking_end);

CREATE INDEX IF NOT EXISTS idx_subsystem_bookings_subsystem_id ON public.subsystem_bookings(subsystem_id);
CREATE INDEX IF NOT EXISTS idx_subsystem_bookings_system_id ON public.subsystem_bookings(system_id);
CREATE INDEX IF NOT EXISTS idx_subsystem_bookings_user_id ON public.subsystem_bookings(user_id);
CREATE INDEX IF NOT EXISTS idx_subsystem_bookings_status ON public.subsystem_bookings(status);
CREATE INDEX IF NOT EXISTS idx_subsystem_bookings_times ON public.subsystem_bookings(booking_start, booking_end);

-- ========================================
-- MIGRATION COMPLETE
-- The booking system is now set up!
-- 
-- NOTE: To enable automatic booking processing,
-- you'll need to set up a cron job or Edge Function
-- to call process_active_bookings() periodically
-- ========================================